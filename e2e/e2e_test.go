...
func project(t *testing.T, fixture string) string {
	...
	dir := t.TempDir()
	data, err := os.ReadFile(filepath.Join("testdata", fixture, "infrena.yml"))
	if err != nil {
		t.Fatal(err)
	}
	writeFile(t, filepath.Join(dir, "infrena.yml"), string(data))
	return dir
}
...
// TestTheWorkflow is the provider guide's checklist (docs/writing-a-provider.md
// section 15), run in order against one project.
func TestTheWorkflow(t *testing.T) {
	dir := project(t, "basic")
	cloud := filepath.Join(dir, ".infrena", "fake-cloud.json")
	...
	t.Run("removing an optional attribute converges", func(t *testing.T) {
		body, _ := os.ReadFile(filepath.Join(dir, "infrena.yml"))
		writeFile(t, filepath.Join(dir, "infrena.yml"), strings.Replace(string(body), "    tags:\n      team: data\n", "", 1))
	...
	t.Run("an injected failure fails the apply, once", func(t *testing.T) {
		body, _ := os.ReadFile(filepath.Join(dir, "infrena.yml"))
		writeFile(t, filepath.Join(dir, "infrena.yml"), string(body)+"\n  extra:\n    type: fake.network\n    cidr: 10.2.0.0/16\n")
		editCloud(t, cloud, func(doc map[string]any) {
	...
	t.Run("removing a resource destroys it", func(t *testing.T) {
		body, _ := os.ReadFile(filepath.Join(dir, "infrena.yml"))
		writeFile(t, filepath.Join(dir, "infrena.yml"), strings.Replace(string(body), "\n  extra:\n    type: fake.network\n    cidr: 10.2.0.0/16\n", "", 1))
...
// TestAWholeResourceReferenceIsFilledInFromTheDeclaration. fake.database's
...
func TestAWholeResourceReferenceIsFilledInFromTheDeclaration(t *testing.T) {
	...
	t.Run("a resource of the wrong type is a compile error", func(t *testing.T) {
		body, _ := os.ReadFile(filepath.Join(dir, "infrena.yml"))
		writeFile(t, filepath.Join(dir, "infrena.yml"), strings.Replace(string(body), "network: ${network}", "network: ${app}", 1))
...
func TestTwoInstancesKeepSeparateClouds(t *testing.T) {
	...
	holds := func(name string) []string {
		data, err := os.ReadFile(filepath.Join(dir, ".infrena", name))
		if err != nil {
...
// TestTheInfrenaUnderTestSpeaksTheManifestsProtocol applies the manifest's own compatibility rules
// to the infrena this suite built, reading what that build says it speaks from
// `infrena version --output`.
//
