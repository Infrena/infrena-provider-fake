...
func TestOperationsOverlapRatherThanSerialise(t *testing.T) {
	...
	readAllAtOnce := func() time.Duration {
		t.Helper()
		start := time.Now()
	...
	// BOTH SIDES MEASURED, rather than one measured against a prediction.
	//
	// This used to compare the concurrent time against resources*delay and fail
	// at half of it. That prediction assumes four reads cost four delays and
	// nothing else: no file loading, no goroutine scheduling, no race detector,
	// and a runner that is not busy. On shared CI none of that holds, and it
	// failed twice — at 150ms, where raising the delay to 400ms was the fix,
	// and again at 400ms in the engine's copy of this test, at 827ms against an
	// 800ms threshold, blocking a release. Raising it a third time is the same
	// losing move, because the quantity being guessed at is the one that varies.
	//
	// Running the same reads one after another measures what serial actually
	// costs here and now. Whatever slows one run slows the other, so the
	// comparison survives a busy runner instead of being its first casualty.
	//
	// Kept in step with github.com/infrena/infrena providers/test, which has
	// the same test for the same reason.
	serial := readEachInTurn()
	concurrent := readAllAtOnce()
