...
`go.mod` requires infrena v0.13.0 and carries no `replace`, so a plain build uses exactly the
release CI builds against:

```bash
go build -o infrena-plugin-fake ./cmd/infrena-plugin-fake
```

If you are working on infrena itself at the same time, lay the two repositories out side by side
and add a **workspace**, which substitutes your checkout for the pinned release:
...
```bash
go work init . ../infrena   # once, from this repository
```

`go.work` is gitignored and never committed, so this is a local decision that no clone inherits —
the default build stays the pinned one. A workspace build compiles **whatever that checkout holds**,
committed or not, so run `git -C ../infrena status` before reading anything into the result, and
`GOWORK=off` (or deleting `go.work`) puts you back on the pinned release for one command or for
good. `go mod tidy` and `go get` ignore the workspace entirely and always fetch, so they need the
credentials above either way.
...
This is `e2e/testdata/basic/infrena.yml` in this repository, and infrena's own e2e suite applies it —
...
A project with no `providers:` block writes its resources to `.infrena/fake-cloud.json`, relative to
the project directory. A named instance (see [Several instances](#several-instances)) that does not
set `cloud:` gets its own file instead: `.infrena/fake-cloud-<instance>.json`. Either way, `cloud:` in
`providers:` overrides the path — relative to the project, or absolute.

After the `apply` above, `.infrena/fake-cloud.json` holds:
...
`.infrena/fake-cloud.json` by hand:
...
`network` declares that it holds a `fake.network`'s `id` (infrena v0.6.0), so
...
This is `e2e/testdata/instances/infrena.yml`. `shared`, on the default instance `main`, is written to
`.infrena/fake-cloud-main.json`; `isolated`, which names `provider: acct2`, is written to
`.infrena/fake-cloud-acct2.json`. Each instance's `cloud:` key, if set, overrides its own file
...
`.infrena/fake-cloud.json`, so an existing cloud file needs no migration. infrena no longer ships a
...
It builds infrena from `$INFRENA_SRC` (default `../infrena`, the sibling checkout), and skips
itself with an `E2E SKIPPED:` line, rather than failing, when that checkout is not present. Run it
before every release.

`$INFRENA_SRC` picks the **host** the suite drives; the module graph picks the **SDK** the plugin is
compiled against. They are separate choices, so a host from one infrena can be paired with an SDK
from another — deliberately useful now and then, but never a result that says "this release works".
CI's gating job keeps them together: the pinned module, and a host checked out at that same tag.
...
- **pinned** (gating): does this plugin work with the infrena it declares? It builds with
  `GOWORK=off` against the tagged infrena module `go.mod` requires, fetched from git, and runs
  gofmt, vet, the unit suite and the compliance suite against an infrena host built from that same
  tag.
- **main** (advisory, never fails the run): has infrena `main` broken us? It runs
  `go work init . ../infrena` against a fresh checkout of infrena's `main` and runs both suites.

Because `go.mod` has no `replace`, the pinned build is simply what a plain checkout does — CI runs
the default rather than having to correct the repository first, and `GOWORK=off` is there so a stray
workspace could never substitute a working tree. The job reads the required version out of `go.mod`
with `go mod edit -json` and refuses anything that is not a release tag — a pseudo-version, or
`v0.0.0`, which names no release — so it stops at that step rather than checking out the wrong
infrena, then asserts that `go list -m` resolves to exactly that tag with no `Replace`.

That build needs infrena's hashes in `go.sum` under `-mod=readonly`. With no `replace` present this
maintains itself: `go mod tidy` resolves infrena as a real module and records them.

## Releasing

`plugin.yaml`, at the repository root, is infrena's plugin manifest — what this plugin is called,
which protocol versions and platforms it supports, and where to find it. As committed today:

```yaml
# plugin.yaml: the manifest infrena reads — what this plugin is, and what it works with.
...
# RAISED TO 0.12.0 ON 2026-09-18, with the move to protocol 5. Every earlier host speaks at
# most 4 and refuses this binary at the handshake, so the floor is forced rather than chosen:
# Supported being a SET protects an old plugin on a new host, and nothing protects the reverse.
# 5 lets a plugin declare the concurrency its API tolerates; this one has no rate limit and
# makes no claim, so the move is about staying on the current protocol rather than about the
# feature.
# Nothing refuses a mismatched host at runtime yet; the check belongs to `infrena plugins install`,
...
**The `infrena:` floor is not the release `go.mod` requires, even when the two read alike.** The floor
is the OLDEST host this binary works with; the `require` (v0.13.0) is the NEWEST it is built and
tested against. The floor moves only when the protocol does — it was last raised with the move to
protocol 5, because a host that speaks at most 4 refuses this binary at the handshake — so between
protocol bumps it sits well below the require. Raising it to match each `require` bump would refuse
hosts the binary runs against perfectly well. Today the floor is documentation and an input to the
compliance suite, not a runtime check: infrena will refuse a plugin whose floor the running build
fails at `infrena plugins install`, which is not built yet.

`protocol: [5]` is the plugin protocol this release's binary speaks. A binary built with infrena's
SDK speaks exactly one: the `pluginproto.Version` of the infrena `go.mod` requires, which v0.3.0
(released before the rename) raised to 2, v0.6.0 raised to 3, v0.7.0 raised to 4 (a discovered
resource may be flagged as created by the cloud itself, which this plugin never does) and v0.12.0
raised to 5 (a plugin may declare the concurrency its API tolerates, which this one does not). So
`protocol:` changes in the same commit as any `require` bump that moves that constant — most do not,
and v0.7.0 through v0.11.1 left it at 4 — and both `internal/fake/manifest_test.go` and
`scripts/release-check` refuse a mismatch. Once released, the value never goes stale — v0.1.1's
binary announces protocol 1 for ever, v0.2.0's announces 2, v0.3.0's announces 3, and a current
infrena still accepts all of them — so a later infrena protocol bump does not force a re-release.
...
1. Runs [CI](#continuous-integration) in full. Only its `pinned` job gates the release: the tests
   run against the infrena release `go.mod` requires, not a working tree.
2. Builds with `GOWORK=off` against that same tagged infrena module, then runs
...
```
$ scripts/release-check v0.5.0
release-check: tag, plugin.yaml and binary all say 0.5.0, and speak protocol [5]
$ echo $?
0
```
...
**[`docs/writing-a-provider.md`](docs/writing-a-provider.md)** is the guide: the two interfaces
method by method, how to model a resource type, errors and retries, testing without a cloud account,
and releasing. It is written for a developer who knows their cloud's API and nothing about infrena,
and it ends with a checklist to run down before you tag a release.
