...
// release's binary speaks, which for an SDK-built binary is exactly one version. Each case keeps
// the version agreeing, so only the protocol can be the refusal: the
// previous release's [1], left behind after an infrena require bump, and the host's whole
// Supported set, [2, 1], which is the wrong answer — that set is what the HOST accepts, not what
// this binary speaks.
