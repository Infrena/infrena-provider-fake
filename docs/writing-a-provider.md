...
This is the only guide, and it explains WHY rather than only what, because the rules only make sense
once you know what the host does with your answers. If you want the what on its own, section 15 is
the checklist. Every claim about infrena
...
starting `pkg/` or `internal/` are in the infrena repository. All other paths are in this one.
...
**infrena:** (v0.7.0) Three things happen to your answer that you should not do yourself:
...
**infrena:** (v0.3.0, `pkg/schema/attribute.go:22-40`)
...
```
Error: cannot describe "fake.database": the fake plugin sent an invalid schema: fake.database: attribute "zone" is Optional without Computed, which says nothing: every attribute that is not Required is already optional. Optional exists to pair with Computed
```

### `Aliases`: other spellings of one attribute

**infrena:** (v0.3.0) `Aliases` lists alternative spellings configuration may use, for example
`Aliases: []string{"cidr", "cidr_block"}` on an attribute declared `CidrBlock`.
...
**infrena:** (v0.6.0, `pkg/schema/attribute.go:14-35` and `:82-89`) An attribute may declare
`References: &schema.Reference{Type, Attribute}`: *this attribute holds that type's attribute*. The fake declares one, on the database's `network`:
...
- **A module boundary carries no reference in either direction.** A module input declares a type, not
  ...
  declares (`refuseWholeResourceInput`/`refuseWholeResourceOutput`, `internal/modules`).
...
the other, so keep declaring both.
...
**`import --generate` uses it too** (infrena v0.7.0), which is another reason to declare every
reference accurately. When the target is in the same import, the generated file refers
...
**infrena:** (v0.6.0, `pkg/schema/attribute.go:91-104`) A `KindMap` attribute may declare
`Fields map[string]schema.Attribute`, the keys the provider knows. Where it does, a path into
...
([section 12](#12-the-manifest-pluginyaml)). This repository did all three at v0.6.x, and has moved with the
protocol since; its manifest always writes whatever its SDK announces. A plugin that declares
neither and stays on an older SDK keeps announcing an older protocol, which every host since
still accepts: `Supported` is a set that keeps the old versions, so a protocol bump never forces
a plugin to re-release.
...
instances (two accounts) make meaningless. It is not a check that this resource references
...
`e2e/testdata/basic/infrena.yml` and `e2e/testdata/instances/infrena.yml`. The README quotes the first
...
1. **Bookkeeping is never sent, so it can't be dropped.** Dependencies, lifecycle and timestamps never
   ...
   The same holds for `lifecycle: ignore_changes: [...]` (infrena v0.3.0). A user lists attributes
   something else owns, such as a task revision a CI pipeline sets on every deploy,
...
(`loader.go:157`, `errors.go:145-151`). Two more rules:
...
- **There is one version per plugin, not per instance.** `plugins:` is keyed by plugin because every
  instance of a plugin shares one process.
...
is in that set, so you don't have to rebuild for every infrena release. Under infrena's own
versioning rules, a minor release may add a protocol version but must keep the previous one, and only
a major release may drop one. If the set no longer includes your version, the user gets an error
naming your plugin, its path, both sides' versions, and which one to upgrade (`internal/pluginhost/errors.go:23-46`).
...
Because of that, `pkg/pluginproto` changes additively, and any removal bumps `protocol` — a new
optional field on the wire is not a protocol bump. `pkg/value` picked up exactly such a field: an
unknown value may now carry the expression that will produce it, added `omitempty` with no version
change. Decode and encode values through `pkg/value` itself, never a hand-rolled or strict decoder —
one that rejects a key it doesn't recognise breaks on the next such change, even though nothing else
about the protocol moved.

---

## 11. Depending on infrena today

`github.com/infrena/infrena` is a public Go module, and its releases are ordinary module versions,
so fetching one needs nothing but the module proxy. The setup this repository uses is **one**
directive — a `require` naming the infrena **release** your plugin supports — and nothing else:

```
// go.mod
require github.com/infrena/infrena v0.13.0
```
...
gains no third-party dependency from them.
...
checkout on disk wasn't what was committed. Run `git -C ../infrena status` before you trust a local
result, and let the gating CI job build the pinned module (`.github/workflows/ci.yml`, job `pinned`).
...
### What CI needs to resolve infrena

infrena is a public module, so nothing special is required to fetch it: the module proxy and
the checksum database both work, and CI needs no credentials. What CI does need is to resolve
the version your plugin claims to build against, and to prove that is what it actually built.

Three things are worth doing in every out-of-tree plugin. This repository writes them inline
in `ci.yml` and `release.yml` rather than in a shared script, because they are short enough
that the indirection cost more than it saved.

1. **`GOWORK=off`** in the job's environment, then build and test under the default
   `-mod=readonly`. This is the one that matters. It is not about credentials; it stops any
   `go.work` on the runner substituting a local checkout for the module your `go.mod`
   requires, which would make CI test something no user will ever run.
2. **Check what resolved**: `go list -m -f '{{.Version}}{{with .Replace}} => {{.Path}}{{end}}'
   github.com/infrena/infrena` must print exactly the required tag. A non-empty `Replace` is
   how you catch one creeping back into `go.mod`.
3. **Commit infrena's hashes to `go.sum`.** `-mod=readonly` needs
   `github.com/infrena/infrena vX.Y.Z h1:…` and its `/go.mod h1:…` line. Do not have CI run
   `go mod tidy` or `go mod download` to write them: a checksum CI generated for itself
   verifies nothing. A no-argument `go mod download` records only the `/go.mod` hashes, which
   is not enough to build.

Two things that are easy to get wrong:

- **A `replace` quietly changes what `go mod tidy` records.** With no `replace` in `go.mod`
  this looks after itself: `go mod tidy` resolves infrena as a real module and records both
  hash lines. Run with a `replace` present, the same command **removes** them, because it
  builds from the directory and has no use for them. That is why a `replace` needs a test and
  a repair script to survive routine maintenance, and dropping it deletes the failure mode
  rather than the alarm for it.
- **Read the version from `go.mod`, not from `go list -m`.** `go mod edit -json` reads the
  file alone and loads no module graph, so it works before any infrena checkout exists. Use
  that one read for both the module and the ref of the e2e host's checkout, so the two can
  never disagree. Refuse anything that is not `vMAJOR.MINOR.PATCH` right there — a
  pseudo-version, or `v0.0.0`, which names no release — so the job stops rather than checking
...
tested this on a scratch copy of its own repository:
...
# pluginproto.Version of the infrena go.mod requires. Declare what your SDK actually announces,
# never a number copied from an example — that is how a manifest drifts from its own binary. It
# changes in the same commit as any require bump that moves the constant, and most do not
# (internal/fake/manifest_test.go and scripts/release-check refuse a mismatch). It never goes
# stale, and a later host protocol bump forces no re-release: the host keeps accepting older
# versions.
protocol: [5]
...
# The OLDEST infrena release this plugin works with: 0.13.0, the release that raised the plugin
# protocol to 5, which is what this binary speaks — an older host refuses it at the handshake.
# This is deliberately NOT go.mod's require, which names the NEWEST infrena the plugin is built
# and tested against. The floor moves only when the protocol does; raising it to match the
# require would refuse hosts this binary works perfectly well with.
infrena: ">= 0.13.0"
...
planned but not built. Loading a plugin from the search path never looks at `plugin.yaml`. Ship it
anyway. Your release gate checks against it (section 13). And once install exists, it will read the
manifest at each release tag, so a release tagged without one stays without one. The plan is to
install such a plugin with only a warning, but then none of the compatibility checks below apply to
it.
...
**infrena:** since v0.3.0, `protocol` is defined as the plugin protocol versions **this release's
binary** can speak. For a plugin built with `pkg/pluginsdk` that is exactly one, the
`pluginproto.Version` of the infrena it was built against, which the SDK puts in the handshake (`pkg/pluginsdk/serve.go:92-96`). A longer list is only
...
only because it is always read at a tag.
...
older infrena that meets newer syntax stops and names the key it didn't understand. That works
because configuration is written and read by the same person, at the same time, on one machine.
...
and tolerate them with a warning for a version they don't.
...
development build of infrena is exempt, as it is from a project's own floor. Validate your
`infrena:` string with `semver.ParseConstraint` (`pkg/semver/semver.go:126`) so you use the same
parser infrena will.

**infrena:** nothing enforces the field at runtime today. The check belongs to `infrena plugins
install`, which is designed but not built, and the handshake doesn't carry it.
`pkg/pluginmanifest.AllowsInfrena` exists, but nothing in infrena calls it yet. A host older than
your floor still loads your plugin. Today the field is documentation, and whatever
...
`>= 0.4.0`; a checkout from before `v0.4.0` fails). This repository's floor names the release
`go.mod` requires, which is the first that speaks the protocol its binary announces.
`internal/fake/manifest_test.go` separately checks the floor refuses hosts below it.
...
- **Checksums.** They can't exist until the binaries are built, so a checked-in manifest can't hold
  ...
  record checksums per platform on the user's side.
...
publish unless they do. This repository's gate also checks one more fact the manifest states about
a release: that `protocol` is exactly the protocol version the same handshake announces.
...
here runs in `.github/workflows/release.yml`'s `release` job, after CI has passed and before
anything is built or published (`release.yml:43-44`).
...
(`.github/workflows/release.yml:49-51`), and then publishes (`release.yml:53-56`). The checksums live
in the release, not the manifest, because they don't exist until the build does. The tests and every
build run against the infrena release `go.mod` requires, fetched as an ordinary module
([section 11](#what-ci-needs-to-resolve-infrena)).
...
infrena's initial AWS resource list. This section is not the design of the whole plugin.
...
inside infrena. Its module path must be outside `github.com/infrena/infrena/`
([section 11](#keep-your-module-path-outside-infrenas)).
...
protocol itself adds no third-party dependency, and `hashicorp/go-plugin` was rejected partly
because gRPC would go far past a dependency budget that so far holds two libraries. Moving the AWS
SDK out of the core module's dependency budget was also the original reason for giving AWS its own
module. Your plugin's `go.mod` can require whatever the cloud needs. infrena's never sees it.

**infrena:** every command starts its plugins, including `validate`, `explain` and `graph`. So a plugin
with expensive startup, such as AWS SDK credential resolution, pays for it on every run.
**Recommendation:** do no network work in `Definitions`, and keep `New` to what configuring an
instance needs.
...
- **There is no ambient `region` (or `account`).** infrena seeds exactly two process variables into
  ...
  any other variable (infrena `internal/variables/resolve.go`, `ProcessVariables`). `region` is an
  ordinary variable name — declare it under `variables:` like any other and write `${var.region}`,
  or use a differently-named one, as the AWS example below does with `${var.aws_region}`. (Before v0.3.0, `region` was reserved but never supplied.)
...
ladder is: the resource's own value, then the instance's `defaults:`, then the schema default. A
map is replaced whole, not merged.
...
- **Changing the instance's default region replaces every resource that relies on it.** `region` is
  ...
  instead of a destroy-and-create (enforced at plan time in `internal/compiler/validate.go`).
...
attribute (infrena v0.5.0). An index is an integer literal only, and an out-of-range index or a
missing key is a compile-time error, not a value known after apply.
...
environment — take that same state-only path with an EMPTY environment, because the
...
Any of this is possible because resolving variables is stages 1-4: it needs no registry, no plugins,
no resources and no modules, which is what makes it available to a command that never compiles — and
why `import <env>`'s own environment was there to pass down all along.
...
provider ID is unique within an account, not across them, so two instances of your plugin can
...
- **Recommendation: declare `References` on every attribute that holds another resource's ID or
  ...
  is the mistake `References` exists to prevent. An attribute that wants an ARN declares
  ...
  take any key. A reference is defined only for a single-valued, top-level attribute. It says nothing
...
- **Satisfied per provider instance, not across the project** — corrected 2026-09-13
  ...
  Two instances are two accounts: a subnet in one account's `us-east-1` no longer satisfies a
...
## 15. Checklist before calling a plugin done

Everything above, as a list you can actually run down before you tag a release. It is ordered
roughly by how early the mistake bites: naming and schema first, then behaviour, then the
convergence properties that only show up against a real cloud, then the release gate.

- [ ] Binary is named `infrena-plugin-<name>` and `Name()` returns `<name>`
...
- [ ] Declaring `References` or `Fields` means requiring infrena >= v0.6.0; your manifest's
      `protocol:` and floor follow whatever release you build against
...
- [ ] Mutating the cloud outside infrena makes the next `plan` propose the change (`refresh` records
...
- [ ] `plugin.yaml` is present, its `name`/`version` agree with the code, and its `protocol` is
      exactly `[pluginproto.Version]` of the infrena you build against
